...
# Define function to train watermark detection model on baseline dataset
def train_baseline(train_dir, val_dir, model_save_path, epochs, batch_size, lr):
    # Define device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Print device being used
    print(f"Baseline training using device: {device}")
    ...
    for epoch in range(epochs):
        ...
        for images, labels in train_loader:
            ...
            # Count number correct predictions
            correct += (preds == labels).sum().item()
        ...
        print(f"[Baseline][Epoch {epoch+1}/{epochs}] Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Loss Convergence (Baseline)")
    plt.legend()
    os.makedirs("results", exist_ok=True)
    plt.savefig("results/loss_convergence_baseline.png")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Accuracy Convergence (Baseline)")
    plt.legend()
    plt.savefig("results/accuracy_convergence_baseline.png")
