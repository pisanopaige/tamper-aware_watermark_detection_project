...
# Define function to finetune the baseline watermark detection model on experimental dataset
def train_finetuned(train_dir, val_dir, pretrained_model_path, model_save_path, epochs, batch_size, lr):
    # Define device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Print device being used
    print(f"Finetuned training using device: {device}")
    ...
    # Initialize baseline watermark detection model
    model = WatermarkDetectionMobileNetV3(pretrained=False)
    model.load(pretrained_model_path, device=device, strict=True)
    ...
    for epoch in range(epochs):
        ...
        print(f"[Finetuned][Epoch {epoch+1}/{epochs}] Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}")
    ...
    print(f"Finetuned model saved at: {model_save_path}")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Loss Convergence (Finetuned)")
    plt.legend()
    os.makedirs("results", exist_ok=True)
    plt.savefig("results/loss_convergence_finetuned.png")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Accuracy Convergence (Finetuned)")
    plt.legend()
    plt.savefig("results/accuracy_convergence_finetuned.png")